   ...
   if (GlobalVariableCheck(distBuy_GV))
     {
      ...
      Print("Предыдущие настройки: дистанция между ордерами: ", DistanceBuy);
     }
   if (GlobalVariableCheck(distSell_GV))
     {
      DistanceSell = GlobalVariableGet(distSell_GV);
      Print("Предыдущие настройки: дистанция между ордерами: ", DistanceSell);
     }
...
int start()
  {
   ...
   /* Блок BUY ордеров */
   if (quantityBuyOrders==0) {SetFirstBuyOrder(); return(0);} //Если нет установленных ордеров, устанавливаем первый.
   if (LastBuyOrderClosed()==true) {ClosedAllBuyOrders(); return(0);} //Если крайний ордер закрыт, то закрываем остальные ордера.
   if (AllowSetNextBuyOrder()==true) SetNextBuyOrder(); //Если условия позвояют устанавливаем следующий ордер с удвоеенным лотом.
   /* Блок SELL ордеров */
   if (quantitySellOrders==0) {SetFirstSellOrder(); return(0);} //Если нет установленных ордеров, устанавливаем первый.
   if (LastSellOrderClosed()==true) {ClosedAllSellOrders(); return(0);} //Если крайний ордер закрыт, то закрываем остальные ордера.
   if (AllowSetNextSellOrder()==true) SetNextSellOrder(); //Если условия позвояют устанавливаем следующий ордер с удвоеенным лотом.
  }
   ...
   for (int i=0; i<OrdersTotal(); i++)
     {
      ...
      quantityOrders++;
      if (OrderOpenTime() > preTime) {preBuyTime = OrderOpenTime(); LastOrderTicket = OrderTicket();}
      switch(OrderType())
        {
         case OP_BUY:
...
void SetFirstBuyOrder()
  {
   ...
   DistanceBuy = calcTP + correctDistance*POINT;
   if (enable_min_distance == true && DistanceBuy < minDistance) DistanceBuy = minDistance*POINT; //если минимальная дистанция активна
   if (DistanceBuy == 0) DistanceBuy = SafeDistance*POINT; //если все же дистанция ноль, то исправляем дистанцию на safedistance
...
void SetFirstSellOrder()
  {
   ...
   DistanceSell = calcTP + correctDistance*POINT;
   if (enable_min_distance == true && DistanceSell < minDistance) DistanceSell = minDistance*POINT; //если минимальная дистанция активна
   if (DistanceSell == 0) DistanceSell = SafeDistance*POINT; //если все же дистанция ноль, то исправляем дистанцию на safedistance
