...
extern int     MMType=1; // “ип ћћ: 0-Lots, 1-как было в 1.2, 2-мартингейл (коэффициент LotExponent)
extern bool    UseClose=false; // закрытие по убытку PipStep. рекомендутс€ false 
extern bool    UseAdd=true; // переоткрытие с новым лотом. лот дл€ переоткрыти€ считаетс€ по LotExponent независимо от MMType рекомендутс€ = true
extern double  LotExponent = 1.667; // умножение лотов в серии по экспоненте дл€ вывода в безубыток. первый лот 0.1, сери€: 0.16, 0.26, 0.43 ...
...
extern double  TakeProfit = 10; // ”ровень прибыли в пипсах от цены открыти€.
 double  Stoploss = 500; // эти три параметра не работают
 double  TrailStart = 10;
 double  TrailStop = 10;
extern double  PipStep = 30; // расто€нию в пипсах убытка на котором открываетьс€ следующий ордер колена.
...
int start(){
   ...
   double LastSellLots;
      ...
      for(cnt=OrdersTotal()-1;cnt>=0;cnt--){// поиск последнего направлени€
