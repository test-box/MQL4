...
int init()
  {
   ...
   if (Distance <= 0 || (initcheck_distance == true && enable_min_distance == true))
     if (Distance < minDistance*POINT && distnow < minDistance*POINT) {Distance = minDistance*POINT; Print("Новая дистанция между ордерами(по минимальной): ", Distance);}
     else if (Distance < minDistance*POINT && distnow > minDistance*POINT) {Distance = distnow; Print("Новая дистанция между ордерами(по расчетной): ", Distance);}
   else if (initcheck_distance == true && Distance < distnow) {Distance = distnow; Print("Новая дистанция между ордерами(по расчетной, мин. отключена): ", Distance);}
...
void SetFirstOrder()
  {
   ...
   Distance = calcDistance() + correctDistance*POINT;
   if (enable_min_distance == true && Distance < minDistance*POINT) Distance = minDistance*POINT; //если минимальная дистанция активна
