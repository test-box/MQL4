...
int init()
  {
   ...
   if ((iBars(NULL, PERIOD_D1) < 11) || (GetLastError() == 4066))
     {Print("Идет обновление исторических данных"); Sleep(10000); Print("Повтор запроса");}
   calcTakeProfit();  // Расчитываем тейкпрофит
   LastOrderTicket = GetLastOrderTicket();
   if (LastOrderTicket == -1)
     {
      FindOrders();
      if (quantityOrders > 0) ClosedAllOrders();
     }
   else FindOrders(); //Поиск существующих ордеров
   dist_GV="ATW_"+Symbol(); Print("dist_GV=", dist_GV); // Устанавливаем название глобальной переменной (Distance) для советника
   if (GlobalVariableCheck(dist_GV))
     {
      ...
      if (Distance == 0) {Distance = calcTP; Print("Дистанция не может быть (0), установлена другая дистанция =", Distance);}
     }
   else {Distance = calcTP; Print("calcTP=", calcTP, "; Дистанция между ордерами: ", Distance);}
...
int start()
  {
   if (iBars(NULL, PERIOD_D1) < 11) {Print("Обновление истории..."); return(4066);}
...
int GetLastOrderTicket()
  {
   int lastticket;
   last_ticket_GV = "ATW_"+Symbol()+"_lastticket";
   if (GlobalVariableCheck(last_ticket_GV))
     {
     lastticket = GlobalVariableGet(last_ticket_GV);
...
void FindOrders()
  {
   ...
   Print("Найдено ордеров:", quantityOrders, "; последний ордер: лоты=", Lots, ", тикет=", LastOrderTicket, ", относительный ТП=", CurrentTP, ", абсолютный ТП=", absTP);
  }
...
void SetFirstOrder()
  {
   ...
   CurrentTP = calcTP + correctTP*POINT;
   if (AutoLot == true) Lots = AccountBalance()/(1000/firstlot);
...
void SendOrder(int command)
  {
   ...
   switch (command)
     {
      case OP_BUY: 
        Print("Ордер Покупки /\: ", "цена аск=", Ask, ", уровень ТП=", Ask+TP, ", относительный ТП=", TP, ", лотов=", Lots);
        ticket = OrderSend(Symbol(),OP_BUY,Lots,Ask,1,0,Ask+TP,CommentOrder,IdNum,0,Blue); 
        break;
      case OP_SELL: 
        Print("Ордер Продажи \/: ", "цена бид=", Bid, ", уровень ТП=", Bid-TP, ", относительный ТП=", TP, ", лотов=", Lots);
