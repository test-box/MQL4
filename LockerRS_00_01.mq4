...
void CheckExistOrders()
  {
   ...
   if (OrderSelect(BSticket, SELECT_BY_TICKET) == true && OrderType() == OP_BUY)  {BS_activated = true; BP = OrderOpenPrice(); B_lot = OrderLots(); StopOrdersActivated = true;}
   if (OrderSelect(SLticket, SELECT_BY_TICKET) == true && OrderType() == OP_SELL) {SL_activated = true; SP = OrderOpenPrice(); S_lot = OrderLots(); StopOrdersActivated = true;}
   if (OrderSelect(BLticket, SELECT_BY_TICKET) == true && OrderType() == OP_BUY)  {BL_activated = true; BP = OrderOpenPrice(); B_lot = OrderLots(); StopOrdersActivated = true;}
   if (OrderSelect(SSticket, SELECT_BY_TICKET) == true && OrderType() == OP_SELL) {SS_activated = true; SP = OrderOpenPrice(); S_lot = OrderLots(); StopOrdersActivated = true;}
   ...
   Comment("Прибыль = ", AccountProfit(), " \r\n",
           ...
           "BUY лоты = ", SumLotsBuy, ", ордеров = ", countBuy, ", текущий лотов = ", B_lot, " \r\n",
           "SELL лоты = ", SumLotsSell, ", ордеров = ", countSell, ", текущий лотов = ", S_lot, " (",  " \r\n",
...
void SetLockers()
  {
   ...
   BSlot = SumLotsSell + inclot - (SumLotsBuy - B_lot); // 1 BuyStop   lot
   SLlot = inclot;                                          // 2 SellLimit lot
   BLlot = inclot;                                          // 3 BuyLimit  lot
   SSlot = SumLotsBuy + inclot - (SumLotsSell - S_lot);  // 4 SellStop  lot
   // buystop order
