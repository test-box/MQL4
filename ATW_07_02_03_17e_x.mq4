...
extern int     correctTP = 36;          // корректировка ТЕЙКПРОФИТА
...
extern double  maxfirstlot = 156.24;    // максимальный объем для первого ордера
extern double  firstlot = 0.05;         // количестов лотов для первого ордера в расчете на DEPOSIT
...
extern bool    enableadd_dist = true;   // включенин/выключение дополнительной дистанции
extern int     stepadd_dist = 4;        // шаг с которого включается добавочная дистанция
extern int     add_dist = 40;           // количество пунктов добавочной дистанции между ордерами
extern bool    ExtraProfit = true;      // Позволяет получить дополнительную прибыль, если были закрыты более preOrders ордеров в одну сторону
extern int     preOrders = 5;           // Количество закрытых ордеров для параметра ExtraProfit
extern double  MultiXtra = 4;           // Множитель лотов для параметра ExtraProfit
extern bool    Limiter = true;          // Включение условий для установки первого ордера
...
extern bool    initcheck_distance = false;  // задействовать проверку дистанции при инициализации, если TRUE
...
extern int     ErrPause = 500;          // пауза после ошибки
...
int init()
  {
   ...
   if (IsTesting())
     {
      ...
      distBuy_GV="ATW_TEST_"+Symbol()+"_distBuy";  //Устанавливаем название глобальной переменной (DistanceBUY)
      distSell_GV="ATW_TEST_"+Symbol()+"_distSell";  //Устанавливаем название глобальной переменной (DistanceSELL)
      lastticketBuy_GV="ATW_TEST_"+Symbol()+"_lastticketBuy";  //Устанавливаем название глобальной переменной (lastticketBUY)
      lastticketSell_GV="ATW_TEST_"+Symbol()+"_lastticketSell";  //Устанавливаем название глобальной переменной (lastticketSELL)
     }
   else if(IsDemo())
     {
      Print("Эксперт работает на ДЕМО-счете. Используются демо-глобальные переменные.");
      distBuy_GV="ATW_DEMO_"+Symbol()+"_distBuy";  //Устанавливаем название глобальной переменной (DistanceBUY)
      distSell_GV="ATW_DEMO_"+Symbol()+"_distSell";  //Устанавливаем название глобальной переменной (DistanceSELL)
      lastticketBuy_GV="ATW_DEMO_"+Symbol()+"_lastticketBuy";  //Устанавливаем название глобальной переменной (lastticketBUY)
      lastticketSell_GV="ATW_DEMO_"+Symbol()+"_lastticketSell";  //Устанавливаем название глобальной переменной (lastticketSELL)
     }
   else
     {
   ...
   //Проверка значения которое было в глобальной переменной Distance
   double distnow = calcDistance() + correctDistance*POINT;
   // По дистанции BUY
   if (DistanceBuy<=0 || (initcheck_distance == true && enable_min_distance == true))
     {
      if (DistanceBuy < minDistance*POINT && distnow < minDistance*POINT) {DistanceBuy = minDistance*POINT; Print("Новая дистанция между BUY ордерами(по минимальной): ", DistanceBuy);}
      else if (DistanceBuy < minDistance*POINT && distnow > minDistance*POINT) {DistanceBuy = distnow; Print("Новая дистанция между BUY ордерами(по расчетной): ", DistanceBuy);}
     }
   else if (initcheck_distance == true && DistanceBuy < distnow) {DistanceBuy = distnow; Print("Новая дистанция между BUY ордерами(по рачетной, мин не установлена): ", DistanceBuy);}
   // По дистанции SELL
   if (DistanceSell<=0 || (initcheck_distance == true && enable_min_distance == true))
     {
      if (DistanceSell < minDistance*POINT && distnow < minDistance*POINT) {DistanceSell = minDistance*POINT; Print("Новая дистанция между SELL ордерами(по минимальной): ", DistanceSell);}
      else if (DistanceSell < minDistance*POINT && distnow > minDistance*POINT) {DistanceSell = distnow; Print("Новая дистанция между SELL ордерами(по расчетной): ", DistanceSell);}
     }
   else if (initcheck_distance == true && DistanceSell < distnow) {DistanceSell = distnow; Print("Новая дистанция между SELL ордерами(по рачетной, мин не установлена): ", DistanceSell);}
   ...
   if (iBars(NULL,PERIOD_D1) == 0 || GetLastError() == 4066)
     {
      ...
      Sleep(15000);
      RefreshRates();
      Print("Пауза для загрузки окончена, если данные не успели подгузиться будет сделана еще одна попытка загрузки.");
      return(false);
     }
   else return(true);
  }
...
void SetFirstBuyOrder()
  {
   ...
   if (AutoLot == true)
     {
      Lots = AccountBalance()/(deposit/firstlot);
      if (ExtraProfit == true && preQuaBuyOrders >= preOrders) Lots = Lots * MultiXtra;
     }
   else Lots = DefaultLots;
   if (Lots > maxfirstlot) Lots = maxfirstlot;
...
void SetFirstSellOrder()
  {
   ...
   if (AutoLot == true)
     {
      Lots = AccountBalance()/(deposit/firstlot);
      if (ExtraProfit == true && preQuaSellOrders >= preOrders) Lots = Lots * MultiXtra;
     }
   else Lots = DefaultLots;
   if (Lots > maxfirstlot) Lots = maxfirstlot;
   if (Lots < minLot) Lots = minLot;
...
bool AllowSetNextBuyOrder()
  {
   ...
   if (quantityBuyOrders >= stepadd_dist && enableadd_dist == true) {if (OrderOpenPrice()-DistanceBuy-add_dist*POINT >= Ask) return(true);}
   else {if (OrderOpenPrice()-DistanceBuy >= Ask) return(true);}
   return(false);
  }
...
bool AllowSetNextSellOrder()
  {
   ...
   if (quantityBuyOrders >= stepadd_dist && enableadd_dist == true) {if (OrderOpenPrice()+DistanceSell+add_dist*POINT <= Bid) return(true);}
   else {if (OrderOpenPrice()+DistanceSell <= Bid) return(true);}
   return(false);
  }
