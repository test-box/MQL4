...
void SetLockers()
  {
   ...
   BSlot = SumLotsSell + inclot - (SumLotsBuy - BSPsumlots);  // 1 BuyStop   lot
   //if (BSlot < 0.01) BSlot = 0.01;
   SLlot = inclot;                                            // 2 SellLimit lot
   BLlot = inclot;                                            // 3 BuyLimit  lot
   SSlot = SumLotsBuy + inclot - (SumLotsSell - SSPsumlots);  // 4 SellStop  lot
   //if (SSlot < 0.01) SSlot = 0.01;
   // buystop order
