...
extern   bool Trade      = true;   // торговать
...
extern double StepGrid   = 0.0013; // расстояние между ступеньками сетки
...
extern   bool AfterClose = true;   // Повторная попытка закрытия всех ордеров
extern    int timer      = 10;     // Задержка перед повторной попыткой закрытия ордеров (сек)
...
int init()
  ...
  if (count == 0)
    {
    if (UseBalance == true) NewBalance = AccountBalance();
    else NewBalance = AccountEquity();
...
void ReBurn()
  ...
  CloseAllOurOrders();
  if (AfterClose == true) {Sleep(1000*timer); CloseAllOurOrders();}
...
void CloseAllOurOrders()
   ...
   for (int t=0; t<OrdersTotal(); t++)  //Цикл поиска ордеров советника, создает массив тикетов ордеров
     {
      ...
      c++;
      OrderTicketMassive[c] = OrderTicket();
     }
   for (int i=1; i <= c; i++)      //Цикл удаления найденных ордеров
     {
     ...
     switch(OrderType())                       // По типу ордера
        {
        case OP_BUY:       Print(" CloseOrder: закрываем BUY");
                           ResetLastError();
                           ...
                           break;
        case OP_SELL:      Print(" CloseOrder: закрываем SELL");
                           ResetLastError();
                           while (OrderCloseTime() != 0) {
                           ...
                           break;
        case OP_BUYSTOP:   Print(" DeleteOrder: закрываем BUYSTOP");
                           ResetLastError();
                           while (_LastError != ERR_INVALID_TICKET) {
                           OrderDelete(OrderTicket(), clrNONE);
                             if (_LastError == ERR_INVALID_TICKET || _LastError == ERR_TRADE_NOT_ALLOWED) break; 
                             RefreshRates();
                             }
                           break;
        case OP_BUYLIMIT:  Print(" DeleteOrder: закрываем BUYLIMIT");
                           ResetLastError();
                           while (_LastError != ERR_INVALID_TICKET) {
                           OrderDelete(OrderTicket(), clrNONE);
                             if (_LastError == ERR_INVALID_TICKET || _LastError == ERR_TRADE_NOT_ALLOWED) break; 
                           ...
                           break;
        case OP_SELLSTOP:  Print(" DeleteOrder: закрываем SELLSTOP");
                           ResetLastError();
                           while (_LastError != ERR_INVALID_TICKET) {
                             OrderDelete(OrderTicket(), clrNONE);
                             if (_LastError == ERR_INVALID_TICKET || _LastError == ERR_TRADE_NOT_ALLOWED) break; 
                           ...
                           break;
        case OP_SELLLIMIT: Print(" DeleteOrder: закрываем SELLLIMIT");
                           ResetLastError();
                           while (_LastError != ERR_INVALID_TICKET) {
                           OrderDelete(OrderTicket(), clrNONE);
                             if (_LastError == ERR_INVALID_TICKET || _LastError == ERR_TRADE_NOT_ALLOWED) break; 
  ...
  Print("Количество удаленных ордеров: ", c);
  }
...
void RefreshOrders()
  ...
  if (Trade == false && CalcOurOrders() == 0) return; 
  int max = MathMax(StepUp, StepDn);
  for (i=1; i<=max; i++)
    {
    if (i <= StepUp)
